...
// 进入详情页时，确保仓库有数据；若缺失则根据路由参数拉取
onMounted(() => {
  const code = route.query.hoscode as string;
  if (code && !hospitalStore.hospitalInfo?.hospital?.hoscode) {
    hospitalStore.getHospital(code);
  }
});
// 监听路由参数变化，动态拉取
watch(() => route.query.hoscode, (val) => {
  const code = val as string;
  if (code) hospitalStore.getHospital(code);
});
</script>
