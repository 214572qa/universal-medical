...
### 🏗️ 核心框架与基础
- **Vue 3 (Composition API)**: 使用组合式 API 提升逻辑复用性与代码组织能力。
- **TypeScript (v5.x)**: 全链路类型支持，减少运行时错误，提升 IDE 智能提示。
- **Vite (v7.x)**: 基于原生 ESM 的超快开发服务器与构建工具。

### 🎨 UI 与样式
- **Element Plus**: 现代化 Vue 3 组件库，负责全局 UI 呈现。
- **Sass (SCSS)**: 预处理器，配合 7-1 模式管理复杂的样式架构。
- **Iconfont & Element Icons**: 提供丰富的业务图标支持。

### 🚦 路由与状态管理
- **Vue Router (v4.x)**: 负责单页应用的路由控制，支持懒加载与导航守卫。
- **Pinia (v3.x)**: 现代化状态管理工具，支持模块化与持久化逻辑。

### 🌐 网络请求与数据模拟
- **Axios**: 二次封装请求库，集成拦截器、统一错误处理与 HTTP 状态码映射。
- **Vite Plugin Mock & Mock.js**: 核心黑科技。通过拦截 Axios 请求并返回本地模拟数据，实现零后端依赖。
- **内存持久化**: Mock 逻辑层支持在内存中维护数据状态（如就诊人列表的增删改查）。

### 辅助工具
- **NProgress**: 页面顶部加载进度条。
- **QRCode**: 用于生成微信支付/预约相关的二维码。
